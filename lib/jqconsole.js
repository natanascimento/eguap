  ...
  JQConsole = (function() {
    ...
    */
    ...
    JQConsole.prototype._CheckKeyCode = function(key_code) {
      ...
      if (!((0 < key_code && key_code < 256)) || isNaN(key_code)) {
        throw new Error('O código da chave deve ser exclusivamente um número entre 0 e 256.');
      }
      return key_code;
    };
    ...
    JQConsole.prototype.RegisterShortcut = function(key_code, callback) {
      ...
      if (typeof callback !== 'function') {
        throw new Error('O retorno de chamada deve ser um função, não ' + callback + '.');
      }
    ...
    */
    ...
    JQConsole.prototype.ClearPromptText = function(clear_label) {
      if (this.state === STATE_OUTPUT) {
        throw new Error('ClearPromptText() não é permitido no estado de saída.');
      }
    ...
    JQConsole.prototype.GetPromptText = function(full) {
      ...
      if (this.state === STATE_OUTPUT) {
        throw new Error('GetPromptText() não é permitido no estado de saída.');
      }
      if (full) {
        this.$prompt_cursor.text('');
    ...
    JQConsole.prototype.SetPromptText = function(text) {
      if (this.state === STATE_OUTPUT) {
        throw new Error('SetPromptText() não é permitido no estado de saída.');
      }
    ...
    JQConsole.prototype.AbortPrompt = function() {
      if (this.state !== STATE_PROMPT) {
        throw new Error('Não é possível abortar o prompt quando não estiver em estado de prompt.'); //pode ser estado de solicitação
      }
